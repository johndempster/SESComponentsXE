...
const DIGD1550_MAX_ANC_CHANNELS = 2;
...
TDIGD1550_ADCtoVolts = Function(
  ...
  function  DIGD1550_LoadProcedure(
   ...
   procedure DIGD1550_CheckError( OK : ByteBool ) ;
...
{ -------------------------------------
  ...
  -------------------------------------}
var
     DIGD1550Path,AxonDLL : String ; // DLL file paths
begin
...
begin
     ...
     if Err <> 0 then begin
        DIGD1550_CheckError(True) ;
        Exit ;
        end ;

     DIGD1550_OpenDevice( DeviceInfo[0].SerialNumber, Err, True ) ;
     if Err <> 0 then begin
        DIGD1550_CheckError(True) ;
        Exit ;
        end ;
...
begin
     ...
     //if not CircularBuffer then Protocol.uFlags := Protocol.uFlags or DIGD1550_FLAG_STOP_ON_TC ;
     ...
     if TriggerMode <> tmWaveGen then begin
        ...
        AOPointer := 0 ;
        DIGD1550_CheckError(DIGD1550_SetProtocol(Protocol)) ;
        // Start A/D conversion
        DIGD1550_CheckError(DIGD1550_StartAcquisition) ;
...
begin
     ...
     if not OK then begin
        DIGD1550_GetLastErrorText(  ErrorMsg, High(ErrorMsg)+1 ) ;
