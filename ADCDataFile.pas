...
type
  ...
  TpClampV5 = packed record { Note. FETCHEX format header block }
	    ...
	    Comment : Array[1..77] of ANSIChar ;
      Labels : Array[1..80] of ANSIChar ;
      Reserved : Array[1..3] of ANSIChar ;
      ChannelNames : Array[0..15,1..10] of ANSIChar ;
      ...
      ADCAmplification : Array[0..15] of single ;
	    ...
	    Units : Array[0..15,1..8] of ANSIChar ;
      end ;

  TABF = packed record
      { Group #1 }
      FileType : Array[1..4] of ANSIChar ;
      ...
      { Group #2 }
	    ...
	    Unused68 : Array[1..4] of ANSIChar ;
      ...
      NumAnnotations : LongInt ;
	    Unused108 : Array[1..2] of ANSIChar ;
      ...
      { Group #6 }
	    ...
	    CellID2 : single ;
	    CellID3 : single ;
	    CreatorInfo : Array[0..15] of ANSIChar ;
	    FileComment : Array[0..55] of ANSIChar ;
      FileStartMillisecs : SmallInt ;
      CommentEnabled : SmallInt ;
	    Unused338 : Array[1..8] of ANSIChar ;

      { Group #7 }
	    ADCPtoLChannelMap : Array[0..15] of SmallInt ;
	    ADCSamplingSeq : Array[0..15] of SmallInt ;
	    ADCChannelName : Array[0..15,1..10] of ANSIChar ;
	    ADCUnits: Array[0..15,1..8] of ANSIChar ;
      ...
      DACChannelName : Array[0..3,1..10] of ANSIChar ;
      DACChannelUnits : Array[0..3,1..8] of ANSIChar ;
      ...
      Unavailable : Array[1..4] of ANSIChar ;
      DigitalDACChannel : SmallInt ;
      Unused1612 : Array[1..6] of ANSIChar ;

      { Group 10 }
      DACFileStatus : single ;
      DACFileOffset : single ;
      Unused1628 : Array[1..2] of ANSIChar ;
      DACFileEpisodeNum : SmallInt ;
      DACFileADCNum : SmallInt ;
      DACFilePath : Array[1..84] of ANSIChar ;
      { Group 11 }
      ...
      Unused1750 : array[1..12] of ANSIChar ;

      { Group 12 }
      ParamToVary : SmallInt ;
      ParamValueList : Array[1..80] of ANSIChar ;
      { Group 13 }
      ...
      Unavailable1866 : array[1..2] of ANSIChar ;
      ...
      ArithmeticK3 : single ;
      ArithmeticK4 : single ;
      ArithmeticOperator : Array[1..2] of ANSIChar ;
      ArithmeticUnits : Array[1..8] of ANSIChar ;
      ArithmeticK5 : single ;
      ArithmeticK6 : single ;
      ArithmeticExpression : SmallInt ;
      Unused1930 : array[1..2] of ANSIChar ;
      ...
      PNInterPulse : single ;
      Unused1954 : array[1..12] of ANSIChar ;
      ...
      LowpassFilterType : Array[0..15] of ANSIChar ;
      HighpassFilterType : Array[0..15] of ANSIChar ;
   ...
   sUnused010 : Array[1..6] of ANSIChar ;
   ...
   sUnused011 : Array[1..30] of ANSIChar ;
   ...
   sUnused012: Array[1..18] of ANSIChar ;
   ...
   sDACFilePath : Array[0..1,0..255] of ANSIChar ;
   sUnused013 : Array[1..12] of ANSIChar ;
   ...
   sUnused014 : Array[1..40] of ANSIChar ;
   ...
   sULParamValueList : Array[0..3,0..255] of ANSIChar ;
   nULRepeat : Array[0..3] of smallint ;
   sUnused015 : Array[1..48] of ANSIChar ;
   ...
   nPNADCSamplingSeq : Array[0..1,0..15] of ANSIChar ;
   ...
   sUnused016a : Array[1..24] of ANSIChar ;

   nAutoAnalyseEnable : SmallInt ;
   sAutoAnalysisMacroName : Array[0..63] of ANSIChar ;
   sProtocolPath : Array[0..255] of ANSIChar ;

   sFileComment : Array[0..127] of ANSIChar;
   GUID : Array[0..15] of ANSIChar ;
   fInstrumentHoldingLevel : Array[0..3] of single ;
   ulFileCRC : Integer ;
   sModifierInfo : Array[0..15] of ANSIChar ;
   sUnused017 : Array[1..76] of ANSIChar ;
   ...
   sUnused018 : Array[1..156] of ANSIChar ;
...
TLDTSegmentHeader = packed record
    ...
    TCFSFileHeader = packed record
	      ...
	      Name : Array[1..14] of ANSIChar ;
	      FileSz : LongInt ;
        TimeStr : Array[1..8] of ANSIChar ;
	      DateStr : Array[1..8] of ANSIChar ;
	      DataChans : SmallInt ;
	      FilVars : SmallInt ;
	      DatVars : SmallInt ;
	      fileHeadSz : SmallInt ;
	      ...
	      CommentStr : Array[1..74] of ANSIChar ;
	      TablePos : LongInt ;
...
TIGORWaveHeader2 = packed Record
  ...
  WavePointer : Cardinal ;           // Used in memory only. Write zero. Ignore on read.
	...
	dataUnits : Array[0..MAX_UNIT_CHARS+1-1] of ANSIChar ;	// Natural data units go here - null if none.
	xUnits : Array[0..MAX_UNIT_CHARS+1-1] of ANSIChar ;	            // Natural x-axis units go here - null if none.
...
TIGORWaveHeader5 = packed Record
	...
	whpad1 : Array[0..5] of ANSIChar ;						// Reserved. Write zero. Ignore on read.
	...
	// SI units
	dataUnits : Array[0..MAX_UNIT_CHARS+1-1] of ANSIChar ;	// Natural data units go here - null if none.
	dimUnits : Array[0..MAXDIMS-1,0..MAX_UNIT_CHARS+1-1] of ANSIChar ;     	// Natural dimension units go here - null if none.
	fsValid : SmallInt ;							// TRUE if full scale values have meaning.
...
TPONEMAHHeader = Packed Record
  ...
  Hours : Byte ;
  UserID : Integer ;
  UserName : Array[0..39] of ANSIChar ;
...
//    NewFile : Boolean ;             // TRUE if newly created file
    ...
    procedure FillCharArray(
              InString : String ;
              var OutArray : Array of ANSIChar ;
...
const
     NumBytesPerSector = 512 ;
var
   Header : array[1..WCPMaxBytesInFileHeader] of ANSIChar ;
...
begin
     ...
     while not Done do begin
         ...
         CopyANSIArrayToString(RecordStatus,RecordStatusArray) ;
...
begin
     ...
     // Size of data record
     NumRecordDataSectors := FNumRecordDataBytes / NumBytesPerSector ;
     AppendDouble( Header, 'NBD=', NumRecordDataSectors ) ;
...
var
   FilePointer : Int64 ;
   cBuf : array[1..20] of ANSIChar ;
   i,ch : Integer ;
   cRecID : array[0..WCPMaxRecordIdentChars-1] of ANSIchar ;
   cRecType : Array[0..WCPMaxRecordTypeChars-1] of ANSIchar ;
begin
...
begin
    ...
    WCPRecordHeader.Value[WCPvFitChan] := WCPRecordHeader.FitChan ;
     ...
     CopyStringToANSIArray( cBuf, WCPRecordHeader.Status ) ;
     FileWrite( FileHandle, cBuf, WCPMaxRecordStatusChars ) ;

     // Record type string (4 chars)
     CopyStringToANSIArray( cBuf, WCPRecordHeader.RecType ) ;
     FileWrite( FileHandle, cBuf, WCPMaxRecordTypeChars ) ;
     ...
     CopyStringToANSIArray( cBuf,WCPRecordHeader.Ident ) ;
     FileWrite( FileHandle, cBuf, WCPMaxRecordIdentChars ) ;
...
type
    ...
    TCDRHeader = packed record         { DOS CDR data file header }
            ...
            Cell : array[0..77] of ANSIChar ;
            FileName : array[0..11] of ANSIChar ;
            dt : single ;
            BitCurrent : single ;
            RangeVolts : single ;
            YUnits : array[0..1] of ANSIChar ;
            TUnits : array[0..1] of ANSIChar ;
            CalCurrent : single ;
            CalTime : single ;
            ...
            RunningMean : single ;
            nEvents : SmallInt ;
            TypeList : array[0..29] of ANSIChar ;
            nTypes : SmallInt ;
...
const
   ...
   msToSecs = 0.001 ;
var
   SPANHeader : Array[0..511] of ANSIChar ;
...
const
   ...
   msToSecs = 0.001 ;
var
   SCANHeader : Array[0..511] of ANSIChar ;
   Header : Array[0..1023] of ANSIChar ;
   i,j,ch : Integer ;
   Value : Single ;
begin
...
// -------------------------------------------
var
   Header : array[1..EDRFileHeaderSize] of ANSIChar ;
   i,ch : Integer ;
...
var
   Header : Array[0..511] of ANSIChar ;
   ch,i : Integer ;
begin
...
var
   Header : array[1..NumWCDFileHeaderBytes] of ANSIChar ;
   i,ch : Integer ;
   NumSamplesInFile : Integer ;
...
begin
    ...
    for i := 0 to High(pc6header.FileComment) do pc6header.FileComment[i] := ' ' ;
    CopyStringToANSIArray( pc6header.FileComment, FIdentLine ) ;
    ...
    for ch := 0 to 15 do begin
        ...
        s := format( 'Ch%d ',[ch] ) ;
        for i := 1 to 4 do pc6header.ADCChannelName[ch,i] := ANSIChar(s[i]) ;
    ...
    for ch := 0 to FNumChannelsPerScan-1 do begin
        ...
        for i := 1 to 8 do if i <= Length(FChannelUnits[ch]) then
            pc6Header.ADCUnits[ch,i] := ANSIChar(FChannelUnits[ch][i]) ;

        for i := 1 to 10 do if i <= Length(FChannelName[ch]) then
            pc6Header.ADCChannelName[ch,i] := ANSIChar(FChannelName[ch][i]) ;
...
begin
    ...
    for ch := 0 to FNumChannelsPerScan-1 do begin
        ...
        CFSChannelDef.ChanName := ANSIString(FChannelName[ch]) ;
        CFSChannelDef.UnitsY := ANSIString(FChannelUnits[ch]) ;
...
begin
    ...
    cSep := ANSIChar(ItemSeparator) ;

    while not Done do begin

        // Read character
        if FileRead( FileHandle, InChar, 1 ) <> 1 then
           begin
           EOF := True ;
           Break ;
           end ;
...
//        outputdebugstring(Pchar(s)) ;
        ...
        if InChar = cSep then
           ...
           while not Done do
              ...
              if (InChar <> #10) and (InChar <> #13) and (not Done) then
                 begin
...
begin
    ...
    if not FASCIISaveRecordsInColumns then
       ...
       for i := 0 to FNumScansPerRecord-1 do
           begin
           t :=  i*FScanInterval ;
           s := format('%.7g',[t]) ;
...
// -------------------------------------------
var
   Header : array[1..CHTFileHeaderSize] of ANSIChar ;
   i,ch : Integer ;
   NumSamplesInFile : Integer ;
   NumMarkers : Integer ;
   MarkerTime : Single ;
...
var
    i : Integer ;
begin
    for i := 0 to Min(Length(Source)-1,High(Dest)) do Dest[i] := ANSIChar(Source[i+1]) ;
    end ;
...
procedure TADCDataFile.AppendDouble(
          var Dest : Array of ANSIChar;
          Keyword : string ;
          Value : Double ) ;
{ --------------------------------------------------------
...
procedure TADCDataFile.ReadDouble(
          const Source : Array of ANSIChar;
          Keyword : string ;
          var Value : Double ) ;
var
...
procedure TADCDataFile.AppendStringToANSIArray(
          var Dest : array of ANSIChar ;
          Source : string ) ;
// --------------------------------------
...
begin
     ...
     iTo := 0 ;
     while ((Dest[iTo] <> #0) and (iTo < High(Dest))) do Inc(iTo) ;
...
procedure TADCDataFile.CopyStringToANSIArray(
          var Dest : array of ANSIChar ;
          Source : string ) ;
// --------------------------------------
// Append string to end of ANSIChar array
...
begin
     ...
     k := 1 ;
     Found := False ;
     while (not Found) and (s < High(Source)) do begin
          if Source[s] = ANSIChar(Keyword[k]) then begin
               k := k + 1 ;
    ...
    begin
        ...
        begin
             Parameter := Parameter + Char(Source[s]) ;
             s := s + 1 ;
             end ;
        end ;
    end ;
...
begin
     ...
     for i := 0 to High(OutArray) do begin
         OutArray[i] := TermChar ;
         if (i < Length(InString)) and (i < High(OutArray)) then OutArray[i] := ANSIChar(InString[i+1]) ;
         end ;
     end ;
